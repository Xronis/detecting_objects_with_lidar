...
def plot_scan_with_bbox(drive, frame, width, length, one_out_of):
    bounding_boxes = {}
    bb_index = 0
    ...
    for d in drive:
        ...
        data = load_raw_forward_data(d, width=width, length=length, one_out_of=one_out_of)
        ...
        if width and length:
            for _ in range(8192):
                point = random.choice(scan)
                if point[0] < length and point[1] < width:
                    roi.append(point)

            scan_traces = _plot_velo(np.array(roi))
        else:
